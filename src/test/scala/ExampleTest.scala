...
case class ExampleTest() extends AnyFunSpec with Matchers {
    ...
    describe("Part 2") {
        ...
        describe("Given a sequence") {
            it("Should yield a valid conf") {
                val parsedGames = Game.parseInput(input)
                parsedGames.head.getMinimumConfiguration shouldEqual Configuration(maxRed = 4, maxGreen = 2, maxBlue = 6)
            }
        }

        describe("Given the full input") {
            it("Should yield 2286") {
                val parsedGames = Game.parseInput(input)
                Game.sumMinimumConfigurations(parsedGames) shouldEqual 2286
            }
        }
    }
